...
class Lstr(object):
    ...
    def __init__(self, *args, **keywds):
        """
        Instantiate a Lstr; pass a value for either 'resource', 'translate', or 'value'.
        (see Lstr help for examples).
        'subs' can be a sequence of 2-member sequences consisting of values and replacements.
        'fallback' can be a resource key that will be used if the main one is not present for
        the current language in place of falling back to the english value ('resource' mode only).
        """
        if args: raise Exception('Lstr accepts only keyword arguments')
        # Basically just store the exact args they passed.
        # ...however if they passed any Lstr values for subs, replace them with that Lstr's dict
        self._args = keywds
        ourType = type(self)
        ...
        # as of protocol 31 we support short key names ('t' instead of 'translate', etc). Convert.
    ...
    if campaign.isSequential() and not force:
        for i in range(len(levels)):
            ...
            if not levels[i].getComplete():
                bsUI.LockedErrorWindow(campaign.getLevel(levelName).getDisplayString(),campaign.getLevel(levels[i].getName()).getDisplayString())
                return False
                            ...
                            if name in namesImported:
                                ...
                                if call is not None and callable(call):
                                    ...
                                    if moduleAPIVersion == ourAPIVersion:
                                        ...
                                        txt = bs.Lstr(resource='apiVersionErrorText',
                                                      subs=[('${NAME}',name),
                                                            ('${VERSION_USED}',str(moduleAPIVersion)),
                                                            ('${VERSION_REQUIRED}',str(ourAPIVersion))])
...
def _getAllTips():
    ...
    #if bs.getEnvironment()['interfaceType'] == 'large' or ('android' in ua and not 'OUYA' in ua and not bsInternal._isRunningOnFireTV()):
...
def _setLanguage(language,printChange=True,storeToConfig=True):
    ...
    if switched and printChange:
        bs.screenMessage(bs.Lstr(resource='languageSetText',subs=[('${LANGUAGE}',bs.Lstr(translate=('languages',language)))]),color=(0,1,0))
...
def _doRemoveInGameAdsMessage():
    ...
    if gLastInGameAdRemoveMessageShowTime is None or (t - gLastInGameAdRemoveMessageShowTime > 1000*60*10):
        ...
        with bs.Context('UI'):
            bs.realTimer(1000,bs.Call(bs.screenMessage,
                                      bs.Lstr(resource='removeInGameAdsText',
...
def _getDefaultLanguage():
    langs = { 'de':'German', 'es':'Spanish', 'it':'Italian', 'nl':'Dutch',
              ...
              'hr':'Croatian', 'hu':'Hungarian', 'be':'Belarussian', 'ro':'Romanian',
              'ko':'Korean', 'fa':'Persian', 'zh':'Chinese', 'tr':'Turkish','id':'Indonesian'}
...
def getResource(resource, fallback=None):
    ...
    Deprecated; you should now use bs.Lstr objects for all translations.
    """
...
def translate(category,s,raiseExceptions=False,printErrors=False):
    ...
    This is conceptually slightly different than bsUtils.getResource() in
    ...
    class InternalMusicPlayer(MusicPlayer):
        ...
        class _PickFolderSongThread(threading.Thread):
            ...
            def run(self):
                try:
                    ...
                    if len(allFiles) == 0: raise Exception(bs.Lstr(resource='internal.noMusicFilesInFolderText').evaluate())
        ...
        def _onPlayFolderCB(self,result,error=None):
            if error is not None:
                errStr = bs.Lstr(resource='internal.errorPlayingMusicText').evaluate().replace(u'${MUSIC}',os.path.basename(result))+u'; '+unicode(error)
                #print errStr
    ...
    class ITunesThread(threading.Thread):
        ...
        def run(self):
            ...
            try: gITunesApp
                ...
                # ourself after
                def doPrint():
                    bs.realTimer(1000,bs.Call(bs.screenMessage,bs.Lstr(resource='usingItunesText'),(0,1,0)))
...
def getTimeString(t,centi=True):
    ...
    if h != 0:
        bits.append('${H}')
        subs.append(('${H}',bs.Lstr(resource='timeSuffixHoursText',subs=[('${COUNT}',str(h))])))
    m = ((t/1000)/60)%60
    if m != 0:
        bits.append('${M}')
    ...
    if centi:
        ...
        if s >= 0.005 or not bits:
            bits.append('${S}')
            subs.append(('${S}',bs.Lstr(resource='timeSuffixSecondsText',subs=[('${COUNT}',('%.2f' % s))])))
    else:
        s = (t/1000 % 60)
        if s != 0 or not bits:
            bits.append('${S}')
...
def runMediaReloadBenchmark():
    ...
    def delayAdd(startTime):
        def foo(startTime):
            bs.screenMessage(_getResource('debugWindow.totalReloadTimeText').replace('${TIME}',str(bs.getRealTime()-startTime)))
            bsInternal._printLoadInfo()
            if bsInternal._getSetting("Texture Quality") != 'High':
                bs.screenMessage(_getResource('debugWindow.reloadBenchmarkBestResultsText'),color=(1,1,0))
    ...
    def __init__(self,position=(390,120),scale=1.0,delay=0,lifespan=None,bright=False):
        ...
        if showTitle:
            ...
            self._titleText = bs.newNode('text',
                                         attrs={'text':bs.Lstr(value='${A}:',subs=[('${A}',bs.Lstr(resource='controlsText'))]),
                                                'hostOnly':True,
                                                'scale':1.1*scale,
                                                'shadow':0.5,
                                                'flatness':1.0,
...
class TipsText(bsGame.Actor):
    ...
    def __init__(self,offsY=100):
        ...
        self.titleNode = bs.newNode('text',
                                    delegate=self,
                                    attrs={'text':bs.Lstr(value='${A}:',subs=[('${A}',bs.Lstr(resource='tipText'))]),
    ...
    def changePhrase(self):
        nextTip = bs.Lstr(translate=('tips',_getNextTip()),subs=[('${REMOTE_APP_NAME}',_getRemoteAppName())])
...
class OnScreenCountdown(bsGame.Actor):
    ...
    def __init__(self,duration,endCall=None):
        ...
        self.inputNode = bs.newNode('timeDisplay',attrs={'time2':duration*1000,'timeMax':duration*1000,'timeMin':0})
...
def _showPostPurchaseMessage():
    ...
    if _gLastPostPurchaseMessageTime is None or curTime-_gLastPostPurchaseMessageTime > 3000:
        ...
        with bs.Context('UI'):
            bs.screenMessage(bs.Lstr(resource='updatingAccountText',fallback='purchasingText'),color=(0,1,0))
...
def _onAccountStateChanged():
    ...
    if bsInternal._getAccountState() == 'SIGNED_IN' and _pendingPromoCodes:
        for code in _pendingPromoCodes:
            bs.screenMessage(bs.Lstr(resource='submittingPromoCodeText'),color=(0,1,0))
...
def isCustomUnicodeChar(c):
    if type(c) is not unicode or len(c) != 1: raise Exception("Invalid Input; not unicode or not length 1")
...
def _configServer():
    """apply server config changes that can always take effect immediately (party name, etc)"""
    ...
    # ones here we can apply immediately.  (could reduce cases where players have
    # to rejoin)
...
def _runServer():
    ...
    if bsInternal._getAccountState() != 'SIGNED_IN':
        print 'WARNING: _runServer() expects to run with a signed in server account'
    ...
    # For now, easiest thing may be to edit _getDefaultFreeForAllPlaylist()
    ...
    # also lets fire off an access check if this is our first time through (and they want a public party)...
    if _gRunServerFirstRun:
        def accessCheckResponse(data):
            ...
            if data is None:
                ...
                if data['accessible']:
                    print 'UDP port',port,'access check successful. Your server appears to be joinable from the internet.'
                else:
                        ...
                        if not _gRunServerPlaylistFetch['sentRequest']:
                            ...
                            def onPlaylistFetchResponse(result):
                                if result is None:
                                    ...
                                    sys.exit(-1)
                                # once we get here we simply modify our config to use this playlist
                                typeName = 'teams' if result['playlistType'] == 'Team Tournament' else 'ffa' if result['playlistType'] == 'Free-for-All' else '??'
                                print 'Playlist \''+result['playlistName']+'\' ('+typeName+') downloaded; running...'
                            ...
                            bsInternal._addTransaction({'type':'IMPORT_PLAYLIST',
                                                        'code':_gRunServerPlaylistFetch['playlistCode'],
                                                        'overwrite':True},
                                                       callback=onPlaylistFetchResponse)
                            bsInternal._runTransactions()

                        # if we got a valid result, forget the fetch ever existed and move on..
