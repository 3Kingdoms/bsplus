...
class Team(object):
    ...
    def __init__(self,teamID=0,name='',color=(1,1,1)):
        ...
        # object.__setattr__(self,'name',bs.uni(name))
...
class Session(object):
    ...
    def handleMessage(self,m):
        'General message handling; can be passed any message object.'
        import bsLobby
        if isinstance(m,bsLobby.PlayerReadyMessage):
            ...
            if m.activity is self._activityRetained:

                # if this activity hasn't begun yet, just make note of what to do after we begin it
                if not m.activity._hasBegun:
                    if not m.activity._shouldEndImmediately or m.force:
                ...
                # the activity has already begun; get ready to end it..
                    ...
                    if (not m.activity._hasEnded) or m.force:
                        ...
                        m.activity._hasEnded = True
                        self._activityEndTimer = bs.Timer(m.delay,bs.Call(self._activityEnd,m.activity,m.results),timeType='net')

        elif isinstance(m,PlayerProfilesChangedMessage):
            # if we have a current activity with a lobby, ask it to reload profile
            with bs.Context(self):
                self._lobby.reloadProfiles()

        else: Session.handleMessage(self,m)
    ...
    # TEMP SANITY CHECK
    def _tempSessionSanityCheck(self):
        if bs.getSession(exceptionOnNone=False) is not self:
...
class Activity(object):
    ...
    # TEMP SANITY CHECK
    def _tempActivitySanityCheck(self):
        if bs.getActivity(exceptionOnNone=False) is not self:
            bs.printError('ERROR: incorrect context; got activity '+str(bs.getActivity(exceptionOnNone=False))+'; wanted '+str(self))
    ...
    def _begin(self,session):
        ...
        for p in session.players:
            if p.exists():
                p.resetInput()
                if p.getTeam() is not None: players.append(p)
        ...
        if session._wantToEnd: session._launchEndActivity()
        else:
...
class EndSessionActivity(Activity):
    ...
    def onBegin(self):
        ...
        bsUtils._callAfterAd(bs.Call(bsInternal._newHostSession,bsMainMenu.MainMenuSession))
...
class GameActivity(Activity):
    ...
    def getConfigDisplayString(cls,config):
        ...
        if 'map' in config['settings']:
            ...
            # s = name+" @ "+bsMap.getLocalizedMapName(bsMap.getFilteredMapName(config['settings']['map']))
        elif 'map' in config:
            s = bs.Lstr(value="${NAME} @ ${MAP}",subs=[('${NAME}',name),('${MAP}',bsMap.getMapDisplayString(bsMap.getFilteredMapName(config['map'])))])
            # s = name+" @ "+bsMap.getLocalizedMapName(bsMap.getFilteredMapName(config['map']))
        else:
    ...
    @classmethod
    def getConfigDescriptionLocalized(cls,config):
        """
        Given a game config dict, return a short description for it.
        This is used when viewing game-lists or showing what game
        is up next in a series.
    ...
    def getInstanceNameLocalized(self):
        ...
        """
        bs.printErrorOnce('deprecated - use getInstanceDisplayString()')
        return self.getNameLocalized(self.settings)
    ...
    def getInstanceScoreBoardNameLocalized(self):
        ...
        """
        bs.printErrorOnce('deprecated - use getInstanceScoreBoardDisplayString()')
        try:
            import bsUI
            if isinstance(self.getSession(),bs.CoopSession):
                # return self.getSession()._campaign.getLevel(bsUI.gCoopSessionArgs['level']).getDisplayNameLocalized()
                return self.getSession()._campaign.getLevel(self.getSession()._campaignInfo['level']).getDisplayNameLocalized()
        except Exception:
            bs.printError('error geting campaign level name')

        return self.getInstanceNameLocalized()
    ...
    def showInfo(self):
        ...
        #name = self.getInstanceNameLocalized()
    ...
    def _showTip(self):
        ...
        if len(self.tips) > 0:
            ...
            # a few subs..
            # tip = tip.replace('${PICKUP}',bs.getSpecialChar('topButton'))
            ...
            t = bs.newNode('text',
                           attrs={'text':tip,
                                  'scale':tipScale,
                                  'maxWidth':900,
                                  # 'position':(basePosition[0]-tipWidth*0.5,basePosition[1]),
            ...
            t2 = bs.newNode('text',
                            ...
                            attrs={'text':tipTitle,
                                   'scale':tipTitleScale,
                                   # 'position':(basePosition[0]-tipWidth*0.5-(20 if icon is None else 82),basePosition[1]+2),
            ...
            if icon is not None:
                img = bs.newNode('image',
                                 attrs={'texture':icon,
                                        # 'position':(basePosition[0]-tipWidth*0.5-40,basePosition[1]+1),
    ...
    def _standardDropPowerup(self,index,expire=True):
        ...
        bsPowerup.Powerup(position=self.getMap().powerupSpawnPoints[index],
                          powerupType=bs.Powerup.getFactory().getRandomPowerupType(),expire=expire).autoRetain()
        ...
        if self._tournamentTimeLimitTime <= 0:
            ...
            n = bs.newNode('text',
                           attrs={'vAttach':'top','hAttach':'center',
                                  ...
                                  'text':bs.Lstr(resource='tournamentTimeExpiredText',fallback='timeExpiredText')
                           })
            bs.playSound(bs.getSound('refWhistle'))
            bsUtils.animate(n,"scale",{0:0.0, 100:1.4, 150:1.2})
